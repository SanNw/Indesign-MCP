...
```json
{
  "mcpServers": {
    "indesign": {
      "command": "npx",
      "args": ["-y", "mcp-indesign"],
      "env": { "INDESIGN_MCP_LANGUAGE": "pt-BR" }
    }
  }
}
```
...
```json
{
  ...
  "mcp": {
    "indesign": {
      ...
      "environment": { "INDESIGN_MCP_LANGUAGE": "pt-BR" },
      "enabled": true
    }
  }
}
...
O runtime usa inglês por padrão. Defina o idioma da interface no ambiente do MCP quando necessário:

```text
INDESIGN_MCP_LANGUAGE=en     # Inglês
INDESIGN_MCP_LANGUAGE=pt-BR  # Português (pt e pt-PT também são aceitos)
INDESIGN_MCP_LANGUAGE=es     # Espanhol
```
